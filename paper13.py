...
class SpeechToTextApp:
    def __init__(self, root):
        ...
        self.predefined_texts = {
            "Text 1": "समय - 						    वार्षिक परीक्षा",
            "Text 2": "                                                                                     कक्षा – 6 विषय – हिन्दी",
            "Text 3": "                                                                                   नोट – सभी प्रश्न अनिवार्य है –"
        }
        self.create_widgets()
    ...
    def insert_predefined_text2(self, text=None):
        ...
        start_index = self.text_box.index(tk.END)  # Get the current end index
        self.text_box.insert(tk.END, text + "\n")  # Append the text
        end_index = self.text_box.index(tk.END)  # Get the new end index
        self.text_box.tag_add("bold", start_index, end_index)  # Add the "bold" tag to the inserted text
        self.text_box.tag_config("bold", font=("Arial", 14, "bold"))  # Configure the "bold" font
        # Now, let's find the index of the beginning and end of the inserted text
        start_inserted_index = start_index + "+1c"  # Skip the newline character
        end_inserted_index = end_index + "-1c"  # Skip the newline character
        # Apply "bold" tag only to the inserted text
        self.text_box.tag_add("bold", start_inserted_index, end_inserted_index)
        self.text_box.tag_config("bold", font=("Arial", 14, "bold"))  # Configure the "bold" font
        ...
        if self.recordings:
            ...
            try:
                ...
                for text in self.recordings:
                    bold_run = doc.add_run(text)  # Create a bold run
                    bold_run.bold = True  # Apply bold format
                doc.save(file_path)
            ...
            try:
                ...
                for line in lines:
                    if line.strip():  # Check if the line is not empty
                        ...
                        if self.text_box.tag_ranges("bold"):  # Check if bold tag is present
                            run.bold = True
