...
class SpeechToTextApp:
    ...
    def save_text(self):
        text_content = self.text_box.get("1.0", tk.END)
        file_name = "speech_to_text.docx"  # Default file name
        if text_content.strip():
            # Get the first few words from the text to use as the file name
            file_name = "_".join(text_content.split()[:5]) + ".docx"
        file_path = os.path.join(os.getcwd(), file_name)
        try:
            doc = Document()  # Create a new Document object
            doc.add_paragraph(text_content)  # Add the text content to the document
