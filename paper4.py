...
class SpeechToTextApp:
    ...
    def create_toolbar(self):
        ...
        remove_last_button = tk.Button(toolbar, text="Remove Last Sentence", command=self.remove_last_sentence)
    ...
    def insert_predefined_text(self):
        ...
        try:
            with open("predefined_text.txt", "rb") as file:
                self.predefined_text = file.read().decode("utf-8")
                self.textbox.insert(tk.END, self.predefined_text + "\n")
                self.recordings = [self.predefined_text]  # Reset the recordings list
    ...
    def remove_last_sentence(self):
        if self.counter > 1:
            ...
            if start_pos:
                ...
                self.counter -= 1
                # Remove the last recorded sentence
                if self.recordings:
                    self.recordings.pop()
        elif self.recordings:  # If there's only one sentence (the predefined text)
            self.textbox.delete("1.0", tk.END)  # Delete the entire text box
            self.recordings = []  # Clear the recordings list
